...
#include "SimulatedGyroscope.h"

#include <stdexcept>

SimulatedGyroscope::SimulatedGyroscope(std::vector<double> times, std::vector<Vector3> readings) {
...
void SimulatedGyroscope::update(double time) {
    for(int i = currentMostRecentReading + 1; i < allReadings.size(); i++){
        if(allReadings.at(i).first <= time){//for any readings that should have happened by now
            ...
            Vector3 calibrated = applyCalibration(allReadings.at(i).second);
            calibratedHistory.emplace_front(allReadings.at(i).first, calibrated);
            history.push_front(allReadings.at(i));
...
bool SimulatedGyroscope::isOutOfNewData() const {
    return currentMostRecentReading == allReadings.size() - 1;
}

SimulatedGyroscope::SimulatedGyroscope(const DataLoader &data) {
    allReadings.reserve(data.getAccelData().size());
    for(auto dataPoint : data.getGyroData()){
